...
public class GUIManager{
		...
		foreach(KeyValuePair<string,GUIProperties> entry in GUIObjects){
			...
			switch(entry.Value.type){
				...
				case GUIType.TextField:
					entry.Value.name = GUI.TextField(new Rect(entry.Value.rect),entry.Value.name,skin.GetStyle(entry.Value.style));
					break;
				default:
					break;
			}	
	...
	public string GetGUIText(string tag){return GUIObjects [tag].name;}
}
